...
javascript: (() => {
  ...
  function __cnt() {
    ...
    let sumAll = 0;
    // 全文字数
    const charAll = parseInt(
      document
        .querySelector("#summary-workInfo-characterCount-all")
        .innerText.replace(/[^\d]/g, ""),
      10
    );
    // 「章とエピソード」の各行に対して実行
    document
      .querySelectorAll(".isShown .episode, .isShown .chapter")
      .forEach((element) => {
        ...
        // 以下、エピソードの行に対して処理
        const epiNum = parseInt(
          element
            .querySelector(".episode-characterCount")
            .innerText.replace(/[^\d]/g, ""),
          10
        );
        ...
        chapterElement.querySelector(
          AVG2_IN_CHAPTER
        ).innerText = `外れ値除外平均${avg2}`;
        // 各エピソード行に設定
        const percent = sumAll / charAll;
        element.style.backgroundColor = `rgb(255, 0, 0, ${percent})`;
        element.querySelector(
          `.episode-feedback-cheerComment`
        ).innerText = `${sumAll}(${(percent * 100).toFixed(1)}%)`;
      });
    ...
    document.querySelectorAll(".isShown .chapter").forEach((element, index) => {
      const sum = parseInt(
        element.querySelector(SUM_IN_CHAPTER).innerText.replace(/[^\d]/g, ""),
        10
      );
      const avg = parseInt(
        element.querySelector(AVG_IN_CHAPTER).innerText.replace(/[^\d]/g, ""),
        10
      );
      const avg2 = parseInt(
        element.querySelector(AVG2_IN_CHAPTER).innerText.replace(/[^\d]/g, ""),
        10
      );
